#!/usr/bin/env python
"""
paper_dashboard_run3.py
========================

Compose paper-ready dashboard figures from a completed Run 3 sweep.

Four products per invocation:

1. Per-dataset dashboard (one PNG per dataset, 2x3 grid). For the
   headline cell (alpha=1, miss=10%) of `dataset`:
   ...
   Output: results/run3/dashboards/<dataset_short>_dashboard.png

2. NEW: Per-dataset accuracy-over-rounds grid (3x3 panels).
       Rows: alpha in {0.1, 1.0, 10.0}
       Cols: missing rate in {0%, 10%, 20%}
       Each panel = mean +/- std accuracy over rounds for all 5 algorithms.
   Output: results/run3/dashboards/<dataset_short>_accuracy_curves.png

3. NEW: Per-dataset Macro-F1-over-rounds grid (3x3 panels).
       Same layout as (2) but F1 is computed per round from the stored
       y_true / y_pred dumps under metrics/seed_<s>/<TOKEN>/.
   Output: results/run3/dashboards/<dataset_short>_f1_curves.png

4. Global hero figure (4x3 panels: 3 datasets across, 4 panel types
   down: accuracy curve, per-class F1 heatmap, FedGen-AMDAE confusion,
   mean-acc bar chart).
   Output: results/run3/dashboards/hero_figure.png
...
DEFAULT_OUTPUT = ROOT / "results" / "run3" / "dashboards"
...
def read_f1_curves(metrics_dir: Path, dataset: str, alpha: float,
    ...
    Returns None when no seed exists or no valid rounds are found.
    """
    import h5py
    import numpy as np
...
def _draw_curve_panel(ax, curves_per_algo: Dict[str, "np.ndarray"],
                      title: str, ylabel: str, ylim: Optional[Tuple[float, float]] = None,
...
def panel_curve_summary(ax, models_dir_per_algo: Dict[str, Path], key: str,
    ...
    """Read `key` (e.g. 'glob_acc') from summary HDF5s and draw."""
...
def compose_per_dataset_dashboard(input_root: Path, output_dir: Path,
    ...
    fig.suptitle(f"{dataset_disp}  --  Run 3 dashboard", fontsize=13)
        ...
        for j, miss in enumerate(MISSING_RATES):
            ...
            if which == "accuracy":
                ...
                for algo in ALGOS_ORDER:
                    ...
                    models_per_algo[algo] = cd / "models"
                # Only put a legend on the top-left panel to avoid clutter.
                curves: Dict[str, "np.ndarray"] = {}
...
def compose_hero_figure(input_root: Path, output_dir: Path,
    ...
    for j, (dataset, dataset_disp, dataset_short) in enumerate(DATASETS_ORDER):
        ...
        panel_curve_summary(axes[0, j], models_per_algo, "glob_acc",
    ...
    fig.suptitle(
        "FedGen-AMDAE  --  hero figure across MNIST / FedISIC / HAM10000",
        fontsize=15)
