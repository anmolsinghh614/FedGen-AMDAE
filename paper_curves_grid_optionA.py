#!/usr/bin/env python
"""
paper_curves_grid_optionA.py
============================

Standalone renderer for the 3x3 (alpha x missing rate) accuracy- and
macro-F1-over-rounds grids for the OPTION A sweep (EMNIST-letters,
UCI HAR, WISDM). This is the same layout `paper_dashboard.py`
produces for Run 3 / CIFAR-10, backported to Option A.

Reads directly from the existing Option A HDF5 checkpoints (no
retraining):
...
Emits, per dataset, into `--output-dir` (default
`results/optionA/dashboards/`):

    <dataset_short>_accuracy_curves.png    # 3 (alpha) x 3 (miss rate) panels
    <dataset_short>_f1_curves.png          # same layout, Macro F1 over rounds
...
Usage::
    ...
    # Default: all 3 Option A datasets, both accuracy and F1 grids
    python paper_curves_grid_optionA.py

    # Only accuracy grids (fast; skips per-round F1 recomputation)
    python paper_curves_grid_optionA.py --no-f1

    # Only a subset of datasets
    python paper_curves_grid_optionA.py --datasets "UCI HAR" WISDM

    # Different result tree
    python paper_curves_grid_optionA.py --input-root results/optionA \\
...
DATASETS_ORDER = [
    ("EMnist-letters", "EMNIST-letters", "emnist"),
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--datasets", nargs="*",
                   ...
                   help="Subset of Option A datasets to render "
                        "(default: all 3).")
    p.add_argument("--no-accuracy", action="store_true",
                   help="Skip the accuracy-over-rounds grids.")
    p.add_argument("--no-f1", action="store_true",
                   help="Skip the F1-over-rounds grids "
                        "(these are the slower ones: they recompute F1 "
