...
def process_data():
    ...
    def process_(mode, tf, ratio=0.1, user_lists=None, agg_user=-1):
        ...
        with open(read_path, 'r') as inf:
            ...
            if mode == 'train':
                if ratio > 1:
                    assert 0 < ratio <= n_users, f"ratio={ratio} must be <= number of train users ({n_users})"
                else:
                    assert 0 < ratio <= 1, f"ratio={ratio} must be in (0,1] for fractional sampling"
        ...
        target_dir = os.path.join(dump_path, mode)  # e.g., ./user25-agg10/train
        os.makedirs(target_dir, exist_ok = True) 
        dump_path = os.path.join(target_dir, f"{mode}.pt")
