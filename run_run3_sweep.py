...
Master orchestrator for the "Run 3" paper sweep, targeting datasets
where FedGen-AMDAE has the strongest inductive advantage (high-D
correlated features + many classes) so the imputer ablation lands
decisively:

    Datasets        : Mnist  ->  FedISIC (ISIC 2019, 8 classes)
                              ->  HAM10000 (ISIC 2018 Task 3, 7 classes)
...
Per-cell namespacing (identical layout to Option A):

    results/run3/<dataset_short>/alpha<a>_miss<m>/<algo>/
...
Usage:
    ...
    python run_run3_sweep.py --stage 1 --device cuda --times 1

    # Only imputer ablation at the headline cell (alpha=1, miss=10%)
    python run_run3_sweep.py --ablation --device cuda

    # Dry-run: print every command without executing
    python run_run3_sweep.py --dry_run --full_pipeline
"""
...
PY = sys.executable
OUT_ROOT = ROOT / "results" / "run3"

# ---------------------------------------------------------------- locked scope
ALPHAS_DEFAULT = [0.1, 1.0, 10.0]
MISSING_RATES_DEFAULT = [0.0, 0.10, 0.20]
ALGOS_DEFAULT = ["FedAvg", "FedProx", "FedDistill", "FedEnsemble", "FedGen"]
# Run order: Mnist (smallest / fastest sanity) -> FedISIC -> HAM10000.
DATASETS_DEFAULT = ["Mnist", "FedISIC", "HAM10000"]
SAMPLING_RATIO = 0.5
N_USERS_TOTAL = 20

# Per-dataset communication-round budget. Mnist is easy (10 classes,
# small features) so 100 rounds is plenty; the medical image datasets
# need the full 200 to converge on a small CNN, similar to how EMNIST
# needed 200 in the Option A sweep.
ROUNDS = {"Mnist": 100, "FedISIC": 200, "HAM10000": 200}
...
class MilestoneRegistry:
    ...
    Writes three files alongside the sweep results:
      ...
      results/run3/_status.md            human-readable, grouped by phase
      results/run3/_status.json          machine-readable, full record list
      results/run3/_status_summary.json  flat snapshot with verdict / totals
...
def _fmt_alpha(alpha: float) -> str:
    """Render alpha consistently with the Dirichlet generators' dirnames
    (e.g. 1.0 -> '1.0', 0.1 -> '0.1')."""
    return str(alpha)
...
def dataset_split_dir(dataset: str, alpha: float) -> Path:
    ...
    a = _fmt_alpha(alpha)
    if dataset == "Mnist":
        # Note: the MNIST generator uses u20c10-* convention (with class count).
        return ROOT / "data" / "Mnist" / \
            f"u{N_USERS_TOTAL}c10-alpha{a}-ratio{SAMPLING_RATIO}"
    if dataset == "FedISIC":
        return ROOT / "data" / "FedISIC" / \
            f"u{N_USERS_TOTAL}-alpha{a}-ratio{SAMPLING_RATIO}"
    if dataset == "HAM10000":
        return ROOT / "data" / "HAM10000" / \
            f"u{N_USERS_TOTAL}-alpha{a}-ratio{SAMPLING_RATIO}"
...
def ensure_split(args: argparse.Namespace, dataset: str, alpha: float) -> bool:
    ...
    All three Run 3 datasets self-bootstrap their raw data from within
    their generate_niid_dirichlet.py, so there is no separate bootstrap
    phase like the Option A sweep needed for UCI HAR."""
    ...
    t0 = time.time()
    if dataset == "Mnist":
        gen = ROOT / "data" / "Mnist" / "generate_niid_dirichlet.py"
        cwd = ROOT / "data" / "Mnist"
        cmd = [PY, str(gen),
               ...
               "--n_class", "10"]
    elif dataset == "FedISIC":
        gen = ROOT / "data" / "FedISIC" / "generate_niid_dirichlet.py"
        cwd = ROOT / "data" / "FedISIC"
        cmd = [PY, str(gen),
               "--n_user", str(N_USERS_TOTAL),
               "--alpha", str(alpha),
               "--sampling_ratio", str(SAMPLING_RATIO)]
    elif dataset == "HAM10000":
        gen = ROOT / "data" / "HAM10000" / "generate_niid_dirichlet.py"
        cwd = ROOT / "data" / "HAM10000"
        cmd = [PY, str(gen),
               "--n_user", str(N_USERS_TOTAL),
               "--alpha", str(alpha),
               "--sampling_ratio", str(SAMPLING_RATIO)]
...
def phase_paper_outputs(args: argparse.Namespace) -> None:
    ...
    table_cmd = [PY, "paper_table_run3.py",
    ...
    if args.dry_run:
        if REGISTRY is not None:
            REGISTRY.info("paper_outputs", "paper_table_run3", "DRY RUN")
    elif rc != 0:
        print("[WARN] paper_table_run3.py failed (rc={}); see log."
              .format(rc), file=sys.stderr)
        if REGISTRY is not None:
            REGISTRY.failed("paper_outputs", "paper_table_run3",
                            f"rc={rc}", dt)
    else:
        if REGISTRY is not None:
            REGISTRY.passed("paper_outputs", "paper_table_run3",
                            f"13 tables -> {OUT_ROOT / 'tables'}", dt)

    dash_cmd = [PY, "paper_dashboard_run3.py",
                "--input-root", str(OUT_ROOT),
    ...
    if args.dry_run:
        if REGISTRY is not None:
            REGISTRY.info("paper_outputs", "paper_dashboard_run3", "DRY RUN")
    elif rc != 0:
        print("[WARN] paper_dashboard_run3.py failed (rc={}); see log."
              .format(rc), file=sys.stderr)
        if REGISTRY is not None:
            REGISTRY.failed("paper_outputs", "paper_dashboard_run3",
                            f"rc={rc}", dt)
    else:
        if REGISTRY is not None:
            REGISTRY.passed("paper_outputs", "paper_dashboard_run3",
                            f"10 figures -> {OUT_ROOT / 'dashboards'}", dt)
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--num_glob_iters_mnist", type=int, default=None,
                   help=f"Override rounds for Mnist (default {ROUNDS['Mnist']}).")
    p.add_argument("--num_glob_iters_fedisic", type=int, default=None,
                   help=f"Override rounds for FedISIC (default {ROUNDS['FedISIC']}).")
    p.add_argument("--num_glob_iters_ham10000", type=int, default=None,
                   help=f"Override rounds for HAM10000 (default {ROUNDS['HAM10000']}).")
    ...
    p.add_argument("--no_auto_download", dest="auto_download",
                   action="store_false",
                   help="Disable automatic download of raw datasets. "
                        "Mnist auto-downloads via torchvision; FedISIC + "
                        "HAM10000 auto-download from ISIC S3.")
...
def main() -> None:
    ...
    if args.num_glob_iters_mnist is not None:
        ROUNDS["Mnist"] = args.num_glob_iters_mnist
    if args.num_glob_iters_fedisic is not None:
        ROUNDS["FedISIC"] = args.num_glob_iters_fedisic
    if args.num_glob_iters_ham10000 is not None:
        ROUNDS["HAM10000"] = args.num_glob_iters_ham10000
    ...
    if args.full_pipeline:
        ...
        print("  python paper_table_run3.py --metric all --imputer_ablation")
        print("  python paper_dashboard_run3.py --headline-alpha 1 "
              "--headline-missing 0.10")
